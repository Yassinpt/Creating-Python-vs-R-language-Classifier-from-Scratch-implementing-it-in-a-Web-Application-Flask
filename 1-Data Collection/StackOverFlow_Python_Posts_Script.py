...
def getQuestionsUrlsPage(pageNumber):
    questionsUrls = []
    url = "https://stackoverflow.com/questions/tagged/python?sort=newest&page={}&pagesize=30".format(pageNumber)
    ...
    if (response != None):
        try:
            ...
            for questionDiv in questionsDiv:
                ...
                print(aTagWhoHasUrl)
                try:
                    url = aTagWhoHasUrl.get(
                        'href')  # la methode het permet de récupérer le lien dans href pour la balise a
...
for i in range(2462, 3501):
    questionsUrls = getQuestionsUrlsPage(i)
    nameFile = "Posts_Python_Page" + "_" + str(i) + ".csv"
